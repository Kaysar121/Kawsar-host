...
# Setup a persistent session for telebot to reduce RemoteDisconnected errors
apihelper.SESSION = requests.Session()
apihelper.SESSION.headers.update({"Connection": "keep-alive"})
...
BOT_TOKEN = "8369678760:AAEkm_rMblagbLMSi7qN37sJITs8xs8iylQ"

# Load thresholds
CPU_THRESHOLD = float(os.environ.get("CPU_THRESHOLD", "90.0"))
MEMORY_THRESHOLD = float(os.environ.get("MEMORY_THRESHOLD", "90.0"))
MAX_RUNNING_PROCESSES = int(os.environ.get("MAX_RUNNING_PROCESSES", "250"))
...
def keep_alive():
    ...
    t.daemon = True
    t.start()
    print("𝐈'𝐌 𝐊𝐀𝐖𝐒𝐀𝐑 𝐇𝐎𝐒𝐓𝐈𝐍𝐆 𝐁𝐎𝐓 server started.")
...
def updates_handler(message):
    bot.send_message(message.chat.id, "📢 Join our channel: t.me/Kawsar93400")

@bot.message_handler(func=lambda m: m.text == "📞 Contact Owner")
def contact_handler(message):
    bot.send_message(message.chat.id, "📞 Contact: @Kawsar9340")
...
def start_bot():
    ...
    while True:
        try:
            ...
            logger.error(f"Polling crashed: %s", e)
            time.sleep(10)  # wait longer before reconnect

# Main execution
if __name__ == "__main__":
    logger.info("Starting 24x7 Kawsar Hosting Bot...")
